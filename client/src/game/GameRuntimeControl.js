...
export const gameRuntimeControlMethods = {
    chooseDefensiveRead(play = this.currentPlay) {
    if (!this.match) return null;
    const defenseSide = this.match.possession === 'home' ? 'away' : 'home';
    const defenseTeam = defenseSide === 'home' ? this.match.homeTeam : this.match.awayTeam;
    const offenseScore = this.match.score[this.match.possession] ?? 0;
    const defenseScore = this.match.score[defenseSide] ?? 0;
    return chooseDefensiveLook({
      down: this.match.down,
      toGo: this.match.toGo,
      yardLine: this.match.yardLine,
      scoreMargin: defenseScore - offenseScore,
      playType: play?.type,
      defenseTeam
    });
  },

    setDefensiveRead(play = this.currentPlay, targetRole = this.target) {
    this.defensiveLook = this.chooseDefensiveRead(play);
    this.readModifier = resolveReadModifier({
      play,
      look: this.defensiveLook,
      targetRole: targetRole || play?.recommendedTarget || 'WR1'
    });
    ...
    this.lineup();
    this.setDefensiveRead(play, this.target);
    this.phase = 'pre-snap';
    this.status = `${play.formation} - ${play.name}. Read ${this.defensiveLook.label}. Space snaps the ball.`;
    this.banner = '';
    this.renderHud();
  },
    snap() {
    ...
    this.lastReadFeedback = this.readModifier?.feedback ?? '';
    this.status = this.lastReadFeedback ? `Ball snapped! ${this.lastReadFeedback}` : 'Ball snapped!';
    this.banner = '';
    this.sound('snap');
    ...
    const readLane = player === this.carrier && this.ballMode === 'held' && this.currentPlay?.type === 'run'
      ? (this.readModifier?.runLane ?? 0)
      : 0;
    const laneMultiplier = clamp(1 + readLane * 0.55, 0.82, 1.16);
    const accel = 9 + player.ratings.acceleration * 0.12;
    const readNudge = readLane !== 0 ? Math.sign(readLane) * 0.18 : 0;
    player.vx = approach(player.vx, (x + readNudge) * max * laneMultiplier, accel * dt);
    player.vz = approach(player.vz, z * max * laneMultiplier, accel * dt);
